...
export const ReceivedReport = () => {
  const { selectedShop, user, permissions } = useAuth();
  const { hasRead, canExport } = getModulePermissions(permissions, user, 'Received Report');
  ...
  // Call POST /StockTransfer/paged API with filters & destinationShopId
  ...
  const normalizedTransfers = useMemo(() => {
    return (rawTransfers || []).map((t: any) => {
      ...
      const receivedItems = items.filter((it: any) => !it.isRejected);
      const totalItems = receivedItems.length;
      const totalGrossWeight = receivedItems.reduce((sum: number, it: any) => sum + num(it.grossWeight || it.stockEntry?.grossWeight), 0);
      const totalNetWeight = receivedItems.reduce((sum: number, it: any) => sum + num(it.netWeight || it.stockEntry?.netWeight), 0);
  ...
  const filteredTransfers = useMemo(() => {
    return normalizedTransfers.filter((t: any) => {
      const dateVal = t.receivedDate || t.transferDate || t.createDate;
      if (fromDate && new Date(dateVal) < new Date(fromDate)) return false;
      if (toDate && new Date(dateVal) > new Date(toDate + "T23:59:59")) return false;

      if (filterShopId) {
        const matchesDest = t.destinationShopId === filterShopId;
        const matchesSource = t.sourceShopId === filterShopId;
        if (!matchesDest && !matchesSource) return false;
      }

      if (statusFilter !== "All" && t.status && !t.status.toLowerCase().includes(statusFilter.toLowerCase())) return false;
      ...
      if (searchTerm.trim()) {
        ...
        const matchesUser = (t.receivedByUserName || "").toLowerCase().includes(kw);
        ...
        if (!matchesNo && !matchesChallan && !matchesSource && !matchesDest && !matchesUser && !matchesItem) return false;
      }
  ...
  const exportData = useMemo(() => {
    const rows: any[] = [];
    filteredTransfers.forEach((t: any) => {
      const dateStr = t.receivedDate || t.transferDate || t.createDate ? format(new Date(t.receivedDate || t.transferDate || t.createDate), "dd/MM/yyyy hh:mm a") : "-";
      ...
      const receivedBy = t.receivedByUserName || "-";
      ...
      if (t.items && t.items.length > 0) {
        t.items.forEach((item: any) => {
          ...
          rows.push({
            ...
            senderShop: sourceShopName,
            receivingShop: destShopName,
  ...
  const exportColumns: ExportColumn[] = [
    ...
    { header: "Sender Shop", key: "senderShop" },
    { header: "Receiving Shop", key: "receivingShop" },
    { header: "Received By", key: "receivedBy" },
    { header: "Status", key: "status" },
    { header: "Product Code", key: "tagNumber" },
    { header: "Product Name", key: "itemName" },
    { header: "Metal", key: "metal" },
    { header: "Category", key: "category" },
    { header: "Kt.", key: "purity" },
  ...
  const handleExportCSV = () => {
    exportToCSV(`Received_Stock_Report_${format(new Date(), "yyyyMMdd")}`, exportColumns, exportData);
    toast.success(`Exported ${exportData.length} received records to CSV!`);
  };

  const handleExportExcel = () => {
    exportToExcel(`Received_Stock_Report_${format(new Date(), "yyyyMMdd")}`, exportColumns, exportData);
    toast.success(`Exported ${exportData.length} received records to Excel!`);
  };
  ...
  const handlePrintPDF = () => {
    ...
    const statCards: StatCardItem[] = [
      {
        label: "Total Received Transfers",
        value: stats.totalTransfers,
        bgColor: "#ecfdf5",
        textColor: "#0f172a",
        iconSvg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#059669" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="m9 11 3 3L22 4"/></svg>`,
      },
      {
      ...
      },
      {
        ...
        bgColor: "#ecfdf5",
        textColor: "#047857",
        iconSvg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#059669" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m16 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="m2 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="M7 21h10"/><path d="M12 3v18"/><path d="M3 7h18"/></svg>`,
      },
    ];
    ...
    <div className="space-y-6 p-4 sm:p-6 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-5 rounded-xl border border-slate-200 shadow-xs">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 flex items-center gap-3">
            <CheckCircle2 className="w-8 h-8 text-emerald-600" /> Total Received Report
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Real-time logs of received stock transfers, itemized weight breakdown, and CSV/Excel exports
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
          <p className="text-xs text-slate-500 font-medium">Total Received Transfers</p>
          <p className="text-xl font-bold text-slate-800">{stats.totalTransfers}</p>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
          <p className="text-xs text-slate-500 font-medium">Items Received</p>
          <p className="text-xl font-bold text-slate-800">{stats.totalItems}</p>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
          <p className="text-xs text-slate-500 font-medium">Gross Weight Received</p>
          <p className="text-xl font-bold text-blue-700">{formatWeight(stats.totalGrossWeight)} g</p>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
          <p className="text-xs text-slate-500 font-medium">Net Weight Received</p>
          <p className="text-xl font-bold text-emerald-700">{formatWeight(stats.totalNetWeight)} g</p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
          <div>
            ...
            <SearchInput
              placeholder="Search Transfer No, Code..."
              value={searchTerm}
          ...
          <div>
            <label className="text-xs font-semibold text-slate-500 mb-1 block">Receiving Shop</label>
            <ShopSelect
              value={filterShopId}
        ...
        <div className="flex justify-between items-center pt-2 border-t border-slate-100 text-xs text-slate-600 font-medium">
          <span className="flex items-center gap-1.5">
            ...
            Showing <strong>{filteredTransfers.length}</strong> received transfer records
          </span>
          {(searchTerm || fromDate || toDate || statusFilter !== "Received" || selectedMetal !== "ALL") && (
            <Button variant="ghost" size="sm" onClick={() => { setSearchTerm(""); setFromDate(null); setToDate(null); setStatusFilter("Received"); setSelectedMetal("ALL"); setPage(1); }}>
      ...
      {/* Received Transfers Accordion List */}
      <div className="bg-white rounded-xl shadow-xs border border-slate-200 overflow-hidden space-y-3 p-4">
        {isLoading ? (
          <div className="p-12 text-center text-slate-500">Loading received transfers...</div>
        ) : displayedTransfers.length === 0 ? (
          <div className="p-12 text-center text-slate-500">No received transfer records found matching criteria.</div>
        ) : (
          displayedTransfers.map((t: any) => {
            ...
            const formattedDate = t.receivedDate || t.transferDate || t.createDate ? format(new Date(t.receivedDate || t.transferDate || t.createDate), "dd MMM yyyy, hh:mm a") : "-";
              ...
              <div key={t.id} className="border border-slate-200 rounded-lg overflow-hidden transition-all shadow-2xs">
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-xs min-w-[220px]">
                    ...
                    <div>
                      ...
                      <span className="font-semibold text-emerald-700">{t.destinationShop?.name || `Shop #${t.destinationShopId}`}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-xs min-w-[140px]">
                    ...
                    <div>
                      ...
                      <span className="font-medium text-slate-800">{t.receivedByUserName || "-"}</span>
                    </div>
                  </div>

                  {/* Weight Summary */}
                  <div className="text-right">
                    <span className="text-xs text-slate-500 block">Items Received</span>
                ...
                {isExpanded && (
                  <div className="bg-white p-3 border-t border-slate-100 overflow-x-auto">
                    {items.length === 0 ? (
                      <div className="p-4 text-center text-xs text-slate-400 italic">No received items in this transfer.</div>
                    ) : (
                      <table className="w-full text-xs text-left border-collapse">
                        <thead>
                          <tr className="bg-slate-100/70 text-slate-600 font-bold border-b border-slate-200">
                            ...
                            <th className="py-2.5 px-3">Kt.</th>
        ...
        {pageSize < 9999 && filteredTransfers.length > pageSize && (
          <div className="flex flex-col sm:flex-row justify-between items-center bg-slate-50 p-4 rounded-lg border border-slate-200 gap-4 mt-2 shadow-2xs">
            <div className="text-xs text-slate-500 font-medium">
              Showing {(page - 1) * pageSize + 1} to {Math.min(page * pageSize, filteredTransfers.length)} of {filteredTransfers.length} received records
            </div>
            <div className="flex items-center gap-2">
